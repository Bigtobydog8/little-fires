...
function LittleFiresApp() {
  ...
  const getItemsForDate = (date) => {
    ...
    allTaskLists.forEach(listName => {
      ...
      const sources = [
        { tasks: allLists[listName] || [], archived: false },
        { tasks: archivedTasks[listName] || [], archived: true }
      ];
  ...
  const autoArchiveCompletedTasks = React.useCallback(() => {
    ...
    const belongsInArchive = (task) => {
      if (!task || !task.completed || !task.completedAt) return false;
      const completedDate = new Date(task.completedAt);
      // A task with an unparseable completedAt stays where it is rather than
      // being swept somewhere the user won't think to look for it.
    ...
    setArchivedTasks(prev => {
      ...
      Object.keys(moved).forEach(listName => {
        const existing = Array.isArray(next[listName]) ? next[listName] : [];
